...
public class DetailActivity extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		chamngon = new ChamNgon();
		chid = getIntent().getExtras().getInt("chid");
		sqliteHelper = new SQLiteHelper(this);

		chamngon = sqliteHelper.getChamNgon(chid);
		...
		textVietnamese.setText(chamngon.getnDungViet());
		textEnglish.setText(chamngon.getnDungAnh());
		textAuthor.setText(chamngon.getTacGia());


		if (chamngon.getYeuthich() == 0) {
		...
		buttonLove.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (chamngon.getYeuthich() == 0){
					chamngon.setYeuthich(1);
					buttonLove.setBackgroundResource(R.drawable.gnome_favorites);
				}else{
					chamngon.setYeuthich(0);
					buttonLove.setBackgroundResource(R.drawable.favorites_add);
				}

				sqliteHelper.updateChamNgon(chamngon);
			}				     
		});
		...
		buttonOtherShare.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				shareIntent.putExtra(Intent.EXTRA_TEXT ,textVietnamese.getText()+ "\n" + 
						textEnglish.getText() + "\n"+ textAuthor.getText() );
	...
	private void onClickShare() {
		...
		System.out.println("Session Permission in detail: " + session.getPermissions());
		
		if (session == null) {
			
			builder.setTitle(R.string.alert_error_title)
			.setMessage(R.string.alert_error_message)
			.setPositiveButton(R.string.alert_error_positiveButton, null);
		} else {
	...
	private void performPublish(PendingAction action) {
		...
		if (session != null) {
			...
			if (hasPublishPermission()) {
				...
				session.openForPublish(new Session.OpenRequest(this).setCallback(null));
